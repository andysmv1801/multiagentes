...
server.port = 8522
# server.launch()  # Comentamos esta línea para evitar que la visualización se lance automáticamente

# Añadir el servidor basado en sockets.
class Server:
    def __init__(self, model, host="127.0.0.1", port=65432):
        self.model = model
    ...
    def run(self):
        ...
        with conn:
            ...
            while True:
                ...
                if not data:
                    break
                message = self.get_agents_info()
                conn.sendall(message)

    def get_agents_info(self):
        agent_info = []
        for agent in self.model.schedule.agents:
            if isinstance(agent, Carro) or isinstance(agent, Semaforo):
                info = {"type": type(agent).__name__, "pos": agent.pos, "dir": agent.direccion if hasattr(agent, 'direccion') else None, "semaforo": agent.en_vertical if hasattr(agent, 'en_vertical') else None}
                agent_info.append(info)
        return json.dumps(agent_info).encode('utf-8')
server_instance = Server(model)
server_instance.run()
